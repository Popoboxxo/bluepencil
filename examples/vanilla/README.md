...
| No host interference | links, form fields and the `[data-bp-ignore]` regions keep working while a mode is active | FR-1.9, FR-12.6 |
...
```bash
# 1. build the library once (writes dist/bluepencil.js and friends)
npm run build
...
`/dist/bluepencil.js` and `/examples/vanilla/` reachable from the same origin, which is the
...
```html
<script src="/dist/bluepencil.js"></script>   <!-- IIFE build → window.bluepencil -->
...
If the build exists but the file at `/dist/bluepencil.js` is an **ES module** (which is what
`scripts/build.mjs` currently emits — the IIFE is written to `dist/bluepencil.iife.js`), a classic
`<script>` tag cannot parse it. `app.js` is a module, so it imports the very same path as a fallback
and hands the namespace to the bootstrap. Once the build emits the IIFE at the documented path, that
fallback simply never runs. Both cases are covered by the notice logic — nothing is faked.
...
> **Before committing this file:** the repository-wide ignore rule `*.bluepencil.json` (in
> `.gitignore`) matches this path. Add a negation (`!examples/vanilla/data/seed.bluepencil.json`)
> if the fixture seed is supposed to be tracked.
...
10. **Bulk delete** (FR-8.2) — select a filter in the panel, run the bulk delete, confirm it → only
    the matching notes disappear, the count is reported, and the markers follow. `Esc` cancels.

Two extras that are cheap to check while the page is open:

* **No host interference** (FR-1.9, FR-12.6) — with a mode active, click the nav links (they
...
| Symptom | Cause |
|---|---|
| Notice "bluepencil is not on this page yet" | `dist/bluepencil.js` is missing — run `npm run build` and reload |
| Console: `Cannot use import statement outside a module`, layer still works | `/dist/bluepencil.js` is the ESM build and `app.js` loaded it as the fallback (see above) |
